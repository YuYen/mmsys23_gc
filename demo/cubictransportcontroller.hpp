...
#include "congestioncontrol/cubic.hpp"

#include "utils/thirdparty/quiche/cubic_bytes.h"

struct CubicTransportCtlConfig : public TransPortControllerConfig
{
public:
    CubicTransportCtlConfig();

    ~CubicTransportCtlConfig() override = default;
    uint32_t maxWnd{ 64 };
    uint32_t minWnd{ 1 };
 ...
 * */
class CubicTransportCtl : public MPDTransportController,
                         public MultiPathSchedulerHandler,
                         public SessionStreamCtlHandler,
                         public std::enable_shared_from_this<CubicTransportCtl>
{
public:

    explicit CubicTransportCtl(std::shared_ptr<TransPortControllerConfig> ctlConfig);

    ~ CubicTransportCtl() override;
...
//    std::shared_ptr<CubicTransportCtlConfig> m_transCtlConfig;/// transport module config
 ...
 * */
class CubicTransportCtlFactory : public TransportControllerFactory
{
public:
    ~ CubicTransportCtlFactory() override = default;
